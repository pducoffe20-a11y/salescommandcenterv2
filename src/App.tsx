...
import {
  ...
  SettingsView,
  StrategyView,
} from "./components/workflow/WorkflowViews";
...
export function App() {
  ...
  return (
    <WorkflowStateProvider navigate={navigate}>
...
function AppShell({
  ...
  routeState: ReturnType<typeof getRouteState>;
  navigate: (view: View, params?: URLSearchParams) => void;
}) {
  ...
  const launchView = useCallback(
    (nextView: View) => navigate(nextView, paramsForView(nextView)),
    [navigate, paramsForView],
  );
    ...
    <div className="app-shell workflow-shell">
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-card">
          <span className="eyebrow">Evidence rule</span>
          <strong>Facts stay separate from inference.</strong>
          <p>Mock data only. All drafts require human review before use.</p>
        </div>
      </aside>
...
function WorkflowRoute({
  ...
  if (view === "home") {
    return <HomeView onLaunch={launchView} commandCenterData={commandCenterData} />;
  }
  ...
  if (view === "prospects") {
    return <StrategyView selected={selectedProspect} onSelect={selectProspect} copy={copy} />;
  }

  if (view === "precall") {
    return <PreCallView copy={copy} />;
  }
  ...
  if (view === "agent-inbox") {
    return (
      <AgentInbox
        promotedData={commandCenterData}
        onPromotedDataChange={setCommandCenterData}
      />
    );
  }

  if (view === "exports") {
    return <ExportsView copy={copy} />;
  }
